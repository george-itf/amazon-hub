...
import { acquireDistributedLock, releaseDistributedLock } from '../utils/distributedLock.js';
...
router.post('/import', requireStaff, requireIdempotencyKey, async (req, res) => {
  ...
  try {
    ...
    lockAcquired = await acquireDistributedLock(lockName, 300000); // 5 minute timeout
    if (!lockAcquired) {
      return errors.conflict(res, 'Order import is already in progress. Please wait for the current import to complete.');
    }
