...
export default function InventoryPage() {
  ...
  const [adjustForm, setAdjustForm] = useState({ quantity: '', reason: 'STOCK_COUNT', note: '', mode: 'delta' });
  ...
  // Undo state for stock adjustments
  const [undoableAdjustment, setUndoableAdjustment] = useState(null); // { movementId, componentSku, delta, createdAt }
  ...
  // Countdown timer for undo window (2 minutes = 120 seconds)
  useEffect(() => {
    if (!undoableAdjustment) {
      setUndoCountdown(0);
      return;
    }

    const UNDO_WINDOW_MS = 2 * 60 * 1000; // 2 minutes
    const createdAt = new Date(undoableAdjustment.createdAt).getTime();
  ...
  function openAdjustModal(component) {
    ...
    setAdjustForm({ quantity: '', reason: 'STOCK_COUNT', note: '', mode: 'delta' });
    setAdjustError(null);
    setConfirmText('');
  }
  ...
  function getEffectiveDelta() {
    ...
    if (adjustForm.mode === 'absolute') {
      const currentStock = adjustModal.component?.total_available || 0;
      return qty - currentStock;
    }
    return qty;
  }

  // Check if adjustment requires confirmation
  function requiresConfirmation() {
    const delta = getEffectiveDelta();
    if (delta === null) return false;
    const currentStock = adjustModal.component?.total_available || 0;
    const absDelta = Math.abs(delta);
    return absDelta > 100 || absDelta > currentStock * 0.5;
  }
  ...
  async function handleAdjustStock() {
    ...
    if (requiresConfirmation() && confirmText !== 'CONFIRM') {
      setAdjustError('Please type CONFIRM to proceed with this large adjustment');
      return;
    }
    setAdjusting(true);
    setAdjustError(null);
    try {
      ...
      const result = await adjustStock(adjustModal.component.id, 'DEFAULT', delta, adjustForm.reason, adjustForm.note || undefined, idempotencyKey);

      // Set this adjustment as undoable (2-minute window)
      if (result?.movement_id) {
        setUndoableAdjustment({
          movementId: result.movement_id,
          componentSku: adjustModal.component.internal_sku,
          componentId: adjustModal.component.id,
          delta: delta,
          createdAt: new Date().toISOString(),
        });
      }

      setSuccessMessage(null); // Clear any previous success message, we'll show the undo banner instead
      closeAdjustModal();
      await load();
    } catch (err) {
  ...
  // Handle undo action
  async function handleUndoAdjustment() {
    if (!undoableAdjustment) return;

    setUndoing(true);
    try {
      await undoStockAdjustment(undoableAdjustment.movementId);
      setSuccessMessage(`Adjustment undone for ${undoableAdjustment.componentSku}`);
      setUndoableAdjustment(null);
      await load();
    } catch (err) {
      setError(err?.message || 'Failed to undo adjustment');
    } finally {
      setUndoing(false);
    }
  }
  ...
  return (
    ...
    >
      <BlockStack gap="400">
        ...
        {/* Undo Banner for recent stock adjustment */}
        {undoableAdjustment && undoCountdown > 0 && (
          <Banner tone="info" onDismiss={() => setUndoableAdjustment(null)}>
            <InlineStack gap="400" blockAlign="center" wrap={false}>
              <BlockStack gap="100">
                <Text variant="bodyMd" fontWeight="semibold">
                  Stock adjusted for {undoableAdjustment.componentSku} ({undoableAdjustment.delta > 0 ? '+' : ''}{undoableAdjustment.delta})
                </Text>
                <InlineStack gap="200" blockAlign="center">
                  <Text variant="bodySm" tone="subdued">
                    Undo available for {Math.floor(undoCountdown / 60)}:{String(undoCountdown % 60).padStart(2, '0')}
                  </Text>
                  <div style={{ width: '100px' }}>
                    <ProgressBar progress={(undoCountdown / 120) * 100} size="small" tone="highlight" />
                  </div>
                </InlineStack>
              </BlockStack>
              <Button
      ...
      {adjustModal.open && (
        <Modal
          ...
          primaryAction={{
            ...
            loading: adjusting,
            disabled: requiresConfirmation() && confirmText !== 'CONFIRM',
          }}
        ...
        >
          <Modal.Section>
            <BlockStack gap="400">
              ...
              <Card>
                <BlockStack gap="300">
                  <Text variant="headingSm">Adjustment Mode</Text>
                  <InlineStack gap="400">
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                      <input
                        type="radio"
                        name="adjustMode"
                        checked={adjustForm.mode === 'delta'}
                        onChange={() => setAdjustForm(f => ({ ...f, mode: 'delta', quantity: '' }))}
                      />
                      <Text>Change by [+/-X]</Text>
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                      <input
                        type="radio"
                        name="adjustMode"
                        checked={adjustForm.mode === 'absolute'}
                        onChange={() => setAdjustForm(f => ({ ...f, mode: 'absolute', quantity: '' }))}
                      />
                      <Text>Set stock to [X]</Text>
                    </label>
                  </InlineStack>
                </BlockStack>
              </Card>
              ...
              <Card>
                <InlineStack gap="400">
                  ...
                  {adjustForm.quantity && !isNaN(parseInt(adjustForm.quantity)) && getEffectiveDelta() !== null && (
                    <>
                      <BlockStack gap="100">
                        ...
                        <Text
                          variant="headingMd"
                          fontWeight="bold"
                          tone={getEffectiveDelta() > 0 ? 'success' : getEffectiveDelta() < 0 ? 'critical' : undefined}
                        >
                          {getEffectiveDelta() > 0 ? '+' : ''}{getEffectiveDelta()}
                        </Text>
                      </BlockStack>
                      <BlockStack gap="100">
                        <Text variant="bodySm" tone="subdued">{adjustForm.mode === 'absolute' ? 'Target' : 'New'} Stock</Text>
                        <Text variant="headingMd" fontWeight="bold">
                          {adjustForm.mode === 'absolute' ? parseInt(adjustForm.quantity) : (adjustModal.component?.total_available || 0) + parseInt(adjustForm.quantity)}
                        </Text>
                      </BlockStack>
                    </>
                  )}
                </InlineStack>
                {adjustForm.mode === 'absolute' && adjustForm.quantity && !isNaN(parseInt(adjustForm.quantity)) && (
                  <div style={{ marginTop: '12px' }}>
                    <Text variant="bodySm" tone="subdued">
                      Current: {adjustModal.component?.total_available ?? 0} → Target: {parseInt(adjustForm.quantity)} (change: {getEffectiveDelta() > 0 ? '+' : ''}{getEffectiveDelta()})
                    </Text>
                  </div>
                )}
              </Card>
              ...
              {requiresConfirmation() && (
                <Banner tone="warning">
                  <BlockStack gap="200">
                    <Text fontWeight="semibold">This is a large adjustment.</Text>
                    <Text>The change of {Math.abs(getEffectiveDelta())} units exceeds safety thresholds. Type 'CONFIRM' below to proceed.</Text>
                    <TextField
              ...
              <FormLayout>
                <TextField
                  label={adjustForm.mode === 'absolute' ? 'Target Stock Level' : 'Quantity Change'}
                  type="number"
                  value={adjustForm.quantity}
                  onChange={(v) => setAdjustForm(f => ({ ...f, quantity: v }))}
                  helpText={adjustForm.mode === 'absolute' ? 'Enter the desired stock level' : 'Positive to add, negative to remove'}
                  placeholder={adjustForm.mode === 'absolute' ? 'e.g., 50' : 'e.g., 10 or -5'}
                  autoComplete="off"
                />
                <Select
                  label="Reason"
                  options={getReasonOptions(ADJUSTMENT_REASON)}
