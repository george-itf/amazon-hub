...
import {
  InvictaConfirmModal,
  useConfirmModal,
} from '../components/ui/InvictaConfirmModal.jsx';
import {
  getShippingRules,
  createShippingRule,
  updateShippingRule,
...
function ShippingRulesCard() {
  ...
  const handleDeleteRule = (ruleId) => {
    ...
    deleteConfirmModal.open({
      ...
      message: `Are you sure you want to delete the shipping rule "${rule?.name || 'this rule'}"?`,
      confirmText: 'Delete',
      variant: 'danger',
      onConfirm: async () => {
        try {
  ...
  return (
    <Card>
      ...
      <InvictaConfirmModal
        ...
        onConfirm={deleteConfirmModal.confirm}
        title={deleteConfirmModal.config.title}
        message={deleteConfirmModal.config.message}
        confirmText={deleteConfirmModal.config.confirmText}
        variant={deleteConfirmModal.config.variant}
        loading={deleteConfirmModal.loading}
        showIdempotencyWarning={false}
      />
    </Card>
  );
}
...
function DemandModelCard() {
  ...
  // Confirmation modal for training
  const trainConfirmModal = useConfirmModal();
  ...
  const handleTrainModel = () => {
    trainConfirmModal.open({
      title: 'Train Demand Model',
      message: 'Train a new demand model? This may take up to 2 minutes. The current model will be replaced once training completes.',
      confirmText: 'Train Model',
      variant: 'warning',
      onConfirm: executeTrainModel,
    });
  };
  ...
  return (
    <Card>
      ...
      <InvictaConfirmModal
        open={trainConfirmModal.isOpen}
        onClose={trainConfirmModal.close}
        onConfirm={trainConfirmModal.confirm}
        title={trainConfirmModal.config.title}
        message={trainConfirmModal.config.message}
        confirmText={trainConfirmModal.config.confirmText}
        variant={trainConfirmModal.config.variant}
        loading={trainConfirmModal.loading}
        showIdempotencyWarning={false}
      />
    </Card>
  );
...
function DataManagementCard() {
  ...
  // Confirmation modals
  const clearPrefsModal = useConfirmModal();
  const resetBomsModal = useConfirmModal();

  const executeClearPreferences = async () => {
    setClearing(true);
  ...
  const handleClearPreferences = () => {
    ...
    clearPrefsModal.open({
      title: 'Clear User Preferences',
      message,
      confirmText: 'Clear Preferences',
      variant: 'warning',
      onConfirm: executeClearPreferences,
    });
  };

  const executeResetAllBoms = async () => {
  ...
  const handleResetAllBoms = () => {
    resetBomsModal.open({
      title: 'Reset All BOM Assignments',
      message:
      ...
      confirmText: 'Reset All BOMs',
      variant: 'danger',
      requiresConfirmation: true,
      confirmationText: 'RESET ALL BOMS',
      onConfirm: executeResetAllBoms,
    });
  };
  ...
  return (
    <Card>
      ...
      <InvictaConfirmModal
        open={clearPrefsModal.isOpen}
        onClose={clearPrefsModal.close}
        onConfirm={clearPrefsModal.confirm}
        title={clearPrefsModal.config.title}
        message={clearPrefsModal.config.message}
        confirmText={clearPrefsModal.config.confirmText}
        variant={clearPrefsModal.config.variant}
        loading={clearPrefsModal.loading}
        showIdempotencyWarning={false}
      />

      {/* Reset All BOMs Confirmation Modal */}
      <InvictaConfirmModal
        open={resetBomsModal.isOpen}
        onClose={resetBomsModal.close}
        onConfirm={resetBomsModal.confirm}
        title={resetBomsModal.config.title}
        message={resetBomsModal.config.message}
        confirmText={resetBomsModal.config.confirmText}
        variant={resetBomsModal.config.variant}
        loading={resetBomsModal.loading}
        requiresConfirmation={resetBomsModal.config.requiresConfirmation}
        confirmationText={resetBomsModal.config.confirmationText}
        showIdempotencyWarning={false}
      />
    </Card>
  );
